...
const contractABI =
  require("../artifacts/contracts/OrganizationEndorser.sol/OrganizationEndorser.json").abi;
...
exports.updateOrganizationProfile = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Error updating organization profile",
        error: error.message,
      });
  }
};
...
exports.getOrganizationCertificates = async (req, res) => {
  try {
    const { address } = req.params;
    const certificates = await Organization.find({
...
exports.endorseCertificate = async (req, res) => {
  try {
    ...
    const contract = new ethers.Contract(address, contractABI, wallet);
    const tx = await contract.endorseCertificate(
      certificateId,
      endorsementDetails
    );
    const receipt = await tx.wait();
    ...
    res
      .status(500)
      .json({ message: "Error endorsing certificate", error: error.message });
  }
};
